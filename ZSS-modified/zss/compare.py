...
print('using modified ZSS with current_max')
...
try:
    ...
    print('using editdist distance')
except ImportError:
    try:
        from editdistance import eval as strdist
        print('using editdist eval')
    except ImportError:
        print('Using custom distance function. This message will only print once after you imported it')
        def strdist(a, b):
            if a == b:
                return 0
            else:
...
def distance(A, B, get_children, insert_cost, remove_cost, update_cost,
             return_operations=False,current_max = None):
        ...
        for x in range(1, m):  # the plus one is for the xrange impl
            for y in range(1, n):
                ...
                if Al[i] == Al[x+ioff] and Bl[j] == Bl[y+joff]:
                    ...
                    if current_max!=None and max(treedists.diagonal()) > current_max:
                       treedists[-1][-1] = None
                       stop_iter = True
                       return 10000
                else:
