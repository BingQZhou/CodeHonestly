...
df5 = pd.DataFrame(arff.loadarff('./data/5year.arff')[0])
df5.shape
...
pred_y = train_and_predict(design_matrix(X), y, clf, design_matrix(X))
accuracy, ber = balanced_error(pred_y, y)
...
clf_2 = LogisticRegression(C = 1, class_weight= 'balanced')

pred2_y = train_and_predict(design_matrix(X), y, clf_2, design_matrix(X))
balanced_error(pred2_y, y);
...
X_train, X_val, X_test, y_train, y_val, y_test = train_validation_test_split(design_matrix(X), y, 0.25, 0.25, shuffled = True)

clf_3 = LogisticRegression(C = 1, class_weight= 'balanced')

train_pred = train_and_predict(X_train, y_train, clf_3, X_train)#clf_3.predict(X_train).reshape(-1, 1)
val_pred = train_and_predict(X_train, y_train, clf_3, X_val)
test_pred = train_and_predict(X_train, y_train, clf_3, X_test)
...
eval_table = LogisticReg_pip(design_matrix(X), y);


# In[984]:


eval_table.plot(kind = 'line')
eval_table
...
clf_5 = LogisticRegression(C = 1)
pred_y5 = train_and_predict(X_train, y_train, clf_3, X_test)
print(X_train.shape)
for b in beta:
    print('beta =', b, ': ', F_score(pred_y5, y_test, beta = b))
...
pca.fit(X_train)
print('shape of kept attrs: ', pca.components_.shape, '\n')
...
Xpca_train_all = np.matmul(X_train, pca.components_.T)
Xpca_valid_all = np.matmul(X_val, pca.components_.T)
Xpca_test_all = np.matmul(X_test, pca.components_.T)

# Xpca_train_all = pca.transform(X_train)
# Xpca_valid_all = pca.transform(X_val)
# Xpca_test_all = pca.transform(X_test)

clf_8 = LogisticRegression(C = 1, class_weight= 'balanced' )
dimensions = np.arange(5, 35, 5)

val_ber, test_ber = [], []

for d in dimensions:

    Xpca_train, y_train = Xpca_train_all[:, :d], y_train
#     print(y_train.shape)
    Xpca_valid, y_val = Xpca_valid_all[:, :d], y_val
    Xpca_test, y_test = Xpca_test_all[:, :d], y_test
    
    clf_8.fit(Xpca_train, y_train)
    
    pred_y_val = clf_8.predict(Xpca_valid).reshape(-1, 1)
    pred_y_test = clf_8.predict(Xpca_test).reshape(-1, 1)
#     print(pred_y_val.shape)
#     print('for dimension = {0}'.format(d))
    val_ber.append(balanced_error(pred_y_val, y_val, False)[1])
    test_ber.append(balanced_error(pred_y_test, y_test, False)[1])

res_8 = pd.DataFrame(data = {
    'validation_BER': val_ber,
    'test BER': test_ber
}, index = dimensions)
